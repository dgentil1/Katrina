...
program main
	...
	foreach city in houston dallas fayetteville {
		build_placebo_plot_inputs, outcomes(`outcome_vars') controls(`control_vars') tr_period(2006) city(`city')
		build_placebo_plot, outcomes(`outcome_vars') controls(`control_vars') stub(`stub_list') city(`city')
	}
    ...
    forval i = 1/`number_educations' {
		...
		build_placebo_plot_inputs, outcomes(`outcome_vars') controls(`control_vars') ///
		    tr_period(2006) city(houston) level(_`education')
		build_placebo_plot, outcomes(`outcome_vars') controls(`control_vars') ///
...
program build_placebo_plot_inputs
	...
	forval i = 1/`number_outcomes' {		
	   ...
	   foreach i of local groups {
			...
			use "../temp/synth_`var'`level'_`i'", clear
			rename _time years
			gen tr_effect_`i' = _Y_treated - _Y_synthetic
			keep years tr_effect_`i'
			drop if missing(years)
			save "../temp/synth_`var'`level'_`i'", replace
		}
	}
	
	forval i = 1/`number_outcomes' {	
		local var: word `i' of `outcomes'
		use "../temp/synth_`var'`level'_`min_unit'", clear
		foreach i of local groups {
			qui merge 1:1 years using "../temp/synth_`var'`level'_`i'", nogen
	}
