...
********************************************************************************
    ...
    foreach city in houston dallas fayetteville {
		`sil' use "${work_asec}controltrends_`city'.dta", clear 
		
		local outcome_vars	= "lr_w_wage emplyd inactive"
		local stub_list = "Log-wage Employment Inactivity"
		
		did_reshape_label, outcomes(`outcome_vars') city(`city')
		...
		forval i = 1/`number_outcomes' {
			...
			did_scm_tables, var(`var') stub(`stub') city(`city')
		}
	}
	...
	forval i = 1/`number_education' {
		...
		foreach city in houston dallas fayetteville {
			`sil' use "${work_asec}controltrends_`city'_`education'.dta", clear 
			...
			forval i = 1/`number_outcomes' {
				...
				did_scm_tables, var(`var') stub(`stub') city(`city') educ_level(`education')
			}
		}
	}
	...
	forval i = 1/`number_education' {
		...
		did_pretrends, outcomes(`outcome_vars') stub(`stub_list') educ_level(`education') educ_stub(`education_title')
		
		did_tables, outcomes(`outcome_vars') controls(`control_vars') educ_level(`education') educ_stub(`education_title')
	}
...
********************************************************************************
    ...
    `sil' reshape long  `outcomes', i(year) j(city)
	...
	`sil' gen `city'=city==0
	`sil' gen synth=city==1

	`sil' gen postkat = (year>=2006)
	`sil' gen synth_sample= (`city'==1 | synth==1)
	`sil' gen did_`city'=postkat*`city'
...
********************************************************************************  
	...
	`sil' eststo: reg `var' `city' postkat did_`city' if synth_sample==1, r 
	
	`sil' esttab using "${tables}did_synth_`var'_`city'`educ_level'.tex", label se ar2 compress replace nonotes ///
...
********************************************************************************  
	...
	foreach sample in treat control {
		...
		if "`educ_level'" != "" {
		    `sil' keep if `educ_level' == 1
		}
			
		`sil' collapse `outcomes' if `sample'==1 [aw=wtsupp], by(year) 
		`sil' gen `sample' = 1
		`sil' save "${temp}trend_`sample'`educ_level'.dta", replace 
		cap saveold "${temp}trend_`sample'`educ_level'.dta", v(12) replace 
	}
	
	`sil' append using "${temp}trend_treat`educ_level'.dta"
	`sil' replace treat=0 if treat ==.
	`sil' replace control=0 if control==.
	...
	forval i = 1/`number_outcomes' {
		...
		`sil' twoway (line `var' year if treat == 1) ///
	...
	`sil' grc1leg `plots', rows(3) legendfrom(check_`outcome1'`educ_level') position(6) /// 
	...
	`sil' graph display, ysize(8.5) xsize(6.5)
	`sil' graph export "${figures}checkslong`educ_level'.png", replace
...
********************************************************************************
	...
	`sil' gen did_treat=postkat*treat
	...
	forval i = 1/`number_outcomes' {
		local var: word `i' of `outcomes'
		`sil' eststo: reg `var' treat postkat did_treat `controls' ///
			  if (treat ==1 | control==1) [aw=wtsupp], r cluster(metcode2) 
	}

	`sil' esttab using "${tables}diffindiff`educ_level'.tex", label se ar2 compress replace nonotes ///
