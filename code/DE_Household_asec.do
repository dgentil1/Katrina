 ...
 ***** Define Program 
  ...
  program define de_household_asec
 ...
 ***** (1) Share of evacuees among households with evacuees
	...
	* Creating the share (share_evac_hh)
	...
	preserve		
		...
		esttab matrix(output) using "../tables/share_hh_asec.tex", replace
		*esttab matrix(output) using "../tables/share_hh_evac_asec.tex", replace
 ...
 ***** (2.a) Share of people with the same race as the evacuee among households with some evacuees
	...
	preserve
		...
		gen ethnic_head = ethnic if relate==101
		...
		save "../temp/ethnic_someevac_asec.dta", replace
	restore	
	
	* Creating the share (share_evac_hhrace)
	
	preserve
		merge m:1 serial using "../temp/ethnic_someevac_asec.dta", nogen keepusing(samerace)
		...
		esttab matrix(output) using "../tables/share_hh_asec.tex", append
		*esttab matrix(output) using "../tables/share_hh_someevac_race_asec.tex", replace
	restore
 ...
 ***** (2.b) Share of people with the same race as the evacuee among households with all evacuees
	...
	preserve
		...
		keep if samerace==1
		bysort serial: keep if _n==1
		save "../temp/ethnic_allevac_asec.dta", replace
	restore	
	
	* Creating the share (share_evac_hhrace)
	
	preserve
		merge m:1 serial using "../temp/ethnic_allevac_asec.dta", nogen keepusing(samerace)
		...
		esttab matrix(output) using "../tables/share_hh_asec.tex", append
		*esttab matrix(output) using "../tables/share_hh_allevac_race_asec.tex", replace
	restore
 ...
 ***** (3) Share of evacuees that are not head of households
	...
	* Creating the share (share_evac_nothead)
	...
	preserve
		...
		esttab matrix(output) using "../tables/share_hh_asec.tex", append
		*esttab matrix(output) using "../tables/share_nohead_evac_asec.tex", replace
	restore
 ...
 *****	were living in the current location in the last year
	...
	* Creating the share (share_nonevac_head)
	...
	preserve
		...
		esttab matrix(output) using "../tables/share_hh_asec.tex", append
		*esttab matrix(output) using "../tables/share_nonevac_head_asec.tex", replace
	restore
	
	
 ***** (5) Share of evacuees with family relationship in households with evacuees	
	
	gen hh_rel = 1*(famrel>=1)
	...
	preserve
		...
		esttab matrix(output) using "../tables/share_hh_asec.tex", append
		*esttab matrix(output) using "../tables/share_evac_hh_rel_asec.tex", replace
	restore
