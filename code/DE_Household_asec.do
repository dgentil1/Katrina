 ...
 ***** Define Program 
  ...
  program define de_hhshares_asec
 ...
 ***** (1) Share of evacuees among households with evacuees
	...
	* Creating the share (share_evac_hh)
	...
	preserve		
		...
		collapse (mean) share_hh_evac = evac hh_obs (sd) share_hh_evac_sd = evac (count) obs = id [aw=wtsupp]
 ...
 ***** (2.a) Share of people with the same race as the evacuee among households with some evacuees
	...
	* Creating the share (share_evac_hhrace)
	...
	preserve
		...
		collapse (mean) share_hh_evac_race = samerace hh_obs (sd) share_hh_evac_race_sd = samerace (count) obs = id [aw=wtsupp]
 ...
 ***** (3) Share of evacuees that are not head of households
	...
	* Creating the share (share_evac_nothead)
	...
	preserve
		...
		collapse (mean) share_nohead_evac = evac_nohead hh_obs (sd) share_nohead_evac_sd = evac_nohead (count) obs = id [aw=wtsupp]
 ...
 *****	were living in the current location in the last year
	...
	* Creating the share (share_nonevac_head)
	...
	preserve
		...
		collapse (mean) share_nonevac_head = nonevac_head hh_obs (sd) share_nonevac_head_sd = nonevac_head (count) obs=id [aw=wtsupp]
