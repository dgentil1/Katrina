  ...
  ***** Define Program 
  ...
  program define did_pretrends
	...
	foreach sample in treat_expanded control {
		use "../derived_asec/CPSASECfinal_did.dta", clear
	...
	forval i = 1/`number_outcomes' {
		...
		twoway (line `var' year if treat_expanded == 1) ///
			   ...
			   xlabel(1999 "99" 2001 "01" 2003 "03" 2005 "05" 2007 "07" 2009 "09" 2011 "11") xscale(range(1999 2011))
	...
	grc1leg `plots', rows(3) legendfrom(check_`outcome1'`educ_level') position(6) /// 
		   ...
		   caption("{it:Source:} CPS March Supplement 1999 - 2011.", size(vsmall))
  ...
  program define did_tables
	...
	forval i = 1/`number_outcomes' {
		...
		eststo: reg `var' treat_expanded postkat did_treat `controls' ///
			   if (treat_expanded ==1 | control==1) [aw=wtsupp], r cluster(metcode2) 
	}

	esttab using "../tables/diffindiff`educ_level'.tex", label se ar2 compress replace nonotes ///
		   title(`educ_stub' Differences in Differences for Treatment vs Control Group, CPS March ASEC Sample \label{tab5}) keep(treat_expanded postkat did_treat) ///
		   mtitles("Log Wage" "Employment Rate" "Inactivity")   ///
		   addnote("{Note:} Observations are weighted using CPS Supplement weights, robust standard errors in parentheses" "are clustered at the metropolitan area level. We include individual level covariates (age," "age-squared, sex,marital status, the interaction of sex and marital status, education, ethnicity, industry and occupation)." "We also use year and metropolitan area fixed effects." "Source: CPS March Supplement 1999 - 2011.")
