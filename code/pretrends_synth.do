...
program main
    ...
    build_pretrends_plot_mcintosh, outcomes(`outcome_vars') controls(`control_vars') ///\
	    city(houston) tr_period(2006) stub(`stub_list')
	build_pretrends_plot, outcomes(`outcome_vars') controls(`control_vars') ///\
			city(houston) tr_period(2006) stub(`stub_list')	
	build_pretrends_plot, outcomes(`outcome_vars') controls(`control_vars') ///\
	...
	forval i = 1/`number_educations' {
		...
		build_pretrends_plots_education, outcomes(`outcome_vars') controls(`control_vars') ///
...
program  build_pretrends_plot_mcintosh
	syntax [if], outcomes(string) controls(string) city(string) tr_period(int) ///
	    stub(string)

	use "../derived/controltrends_`city'.dta", clear

	merge 1:1 year using "../temp/mcintoshcontrol.dta", nogen
	save "../derived/controltrends_`city'.dta", replace
...
program  build_pretrends_plots_education
	syntax, outcomes(string) controls(string) city(string) tr_period(int) ///
	    stub(string) level(string) title(string)

	use "../derived/controltrends_`city'`level'.dta", clear
