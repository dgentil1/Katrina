...
********************************************************************************  
    ...
    `sil' use "${temp}donorpool_`city'`level'.dta", clear
	...
	`sil' levelsof metcode2, local(groups)
	...
	forval i = 1/`number_outcomes' {		
		...
		foreach i of local groups {
			...
			`sil' use "${temp}synth_`var'`level'_`i'.dta", clear
			rename _time years
			`sil' gen tr_effect_`i' = _Y_treated - _Y_synthetic
			keep years tr_effect_`i'
			`sil' drop if missing(years)
			`sil' save "${temp}synth_`var'`level'_`i'.dta", replace
	...
	forval i = 1/`number_outcomes' {	
		local var: word `i' of `outcomes'
		`sil' use "${temp}synth_`var'`level'_`min_unit'.dta", clear
		foreach i of local groups {
			`sil' merge 1:1 years using "${temp}synth_`var'`level'_`i'.dta", nogen
	}
		
		`sil' save "${temp}allsynth_`var'_`city'`level'.dta", replace
		cap saveold "${temp}allsynth_`var'_`city'`level'.dta", v(12) replace
		
		rename tr_effect_`trunit' `city'
		keep year `city'
		
		`sil' save "${temp}`var'_`city'`level'.dta", replace
...
******************************************************************************** 
	...
	forval i = 1/`number_outcomes' {	
	    ...
	    `sil' use "${temp}allsynth_`var'_`city'`level'.dta", clear
        `sil' merge 1:1 year using "${temp}`var'_`city'`level'.dta", nogen
		`sil' sum `city' if years < 2006
		local bound = 5*(abs(r(max)) + abs(r(min)))
		local lp
	    foreach i of local groups {
		    `sil' sum tr_effect_`i' if years < 2006
		...
		`sil' graph export "${figures}placebo_`var'`level'", replace as(eps)
	}
	...
	`sil' graph combine `plots', rows(3) graphregion(color(white)) ysize(8) xsize(6.5) ///
	...
	`sil' graph export "${figures}placebo`level'`city'", replace as(eps)
