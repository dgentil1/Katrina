...
********************************************************************************  
	...
	`sil' merge 1:1 year using "${temp}mcintoshcontrol.dta", nogen
	`sil' save "${work_asec}controltrends_`city'.dta", replace
	...
	forval i = 1/`number_outcomes' {
		...
		`sil' twoway (line `city'_`outcome_var' year, lcolor(navy) lwidth(thick)) ///
	...
	`sil' grc1leg `plots', rows(3) legendfrom(trend_`outcome1') position(6) /// /* cols(1) or cols(3) */
	...
	`sil' graph export "${figures}alltrends_`city'mcintosh.png", replace
...
********************************************************************************    
	...
	`sil' grc1leg `plots', rows(3) legendfrom(trend_`outcome1') position(6) /// /* cols(1) or cols(3) */
		  ...
		  size(vsmall)) caption("{it:Source:} CPS March Supplement 1996 - 2014.", size(vsmall))
	`sil' graph display, ysize(8.5) xsize(6.5)
	`sil' graph export "${figures}alltrends_`city'.png", replace
...
********************************************************************************    
	...
	`sil' use "${work_asec}controltrends_`city'`level'.dta", clear
	...
	`sil' grc1leg `plots', rows(3) legendfrom(`plot1') position(6) /// /* cols(1) or cols(3) */
		  graphregion(color(white)) title({bf:`title' - Outcome time trends}, color(black) size(small)) ///
		  note("{it:Note:} Each figure shows the outcome variable for `city_legend' (blue solid line)and Synthetic control (dashed line)"" in the period 1994-2014. The top figure shows the graph for the""logarithm of weekly wages, the figure in the middle shows it for employment and the bottom figure for""inactivity. The vertical line is depicted for year 2005.", ///
		  size(vsmall)) caption("{it:Source:} CPS March Supplement 1996 - 2014.", size(vsmall))
	`sil' graph display, ysize(8.5) xsize(6.5)
	`sil' graph export "${figures}alltrends_`city'`level'.png", replace
